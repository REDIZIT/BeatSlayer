...
namespace IngameDebugConsole
{
	...
	public class DebugLogManager : MonoBehaviour
	{
        ...
        public Text fpsText;
        float minFps = 120;
        float currentMinFps = 120;
        float minFpsTime = 2;

        private void Update()
        {
            float smoothFps = 1f / Time.smoothDeltaTime;
            if (smoothFps < minFps && Time.time > 1)
            {
                minFps = smoothFps;
            }
            if (minFpsTime <= 0)
            {
                minFpsTime = 2;
                currentMinFps = minFps;
                minFps = 120;
            }
            else minFpsTime -= Time.deltaTime;
            fpsText.text = Mathf.RoundToInt(smoothFps) + "fps";
            if (Time.deltaTime > 1 || Time.deltaTime <= 0)
            {
                fpsText.text += " <color=red>(" + Time.deltaTime + " delta time)</color>";
            }

            fpsText.text += @"
" + "Min fps: " + Mathf.RoundToInt(currentMinFps);
        }
