...
public class ShowOffEmoji : MonoBehaviour {
    ...
    void Start ()
    {
        ...
        StartCoroutine(this.SetUITextThatHasEmoji(this.receipeText, "Hey, there are some emojies : \U0001F3B5 \U0001F3B6 \U0001F3B7 \U0001F3B8 \U0001F3B9 \U0001F3BA and this is new text after"));
    }
    ...
    public IEnumerator SetUITextThatHasEmoji(Text textToEdit, string inputString)
    {
        ...
        for (int j = 0; j < emojiReplacements.Count; j++)
        {
            ...
            //newRawImage.transform.localScale = new Vector3(textToEdit.fontSize / 80f, textToEdit.fontSize / 80f, textToEdit.fontSize / 80f);
