...
public class GameScript : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        try
        {
            fullTrackName = LCData.author + "-" + LCData.name;

            if (!LCData.isBossLevel)
            {
    ...
    void InitAudio()
    {
        ...
        //if (LCData.loadingType == LCData.LoadingType.AudioFile) spectrumAsource.pitch = pitch;
        pitchSlider.value = (pitch - 1) * 10;
        OnPitchChanged();

        //asource.clip = LCData.aclip;
        audioManager.SetClip(LCData.aclip);
        if (LCData.loadingType == LCData.LoadingType.AudioFile) audioManager.spectrumAsource.clip = LCData.aclip;
        clipTime = SplitTime(LCData.aclip.length);

        Debug.Log("Audio Bytes is null? " + (LCData.project.audioFile == null));
    ...
    IEnumerator Start()
    {
        ...
        {
            //asource.Pause();
            //if (LCData.loadingType == LCData.LoadingType.AudioFile) spectrumAsource.Pause();
        }
        gameStarting = false;
    }
    ...
    private void Update()
    {
        ...
        //{
        //    camRotation = Input.mousePosition / 300;
        //}
        //else
        //{
        //    Vector3 rawVector = new Vector3(Input.acceleration.x * -20, Input.acceleration.y * 20, Input.acceleration.z); ;
        //    camRotation += (rawVector - camRotation) / 50;
        //}
        //transform.localEulerAngles = new Vector3(camRotation.y, 0, 0);
        //transform.parent.parent.eulerAngles = new Vector3(0, 0, camRotation.x * 5);
        ...
        int[] curTime = SplitTime(audioManager.asource.time);
...
" + noArrows;
    ...
    public IEnumerator Unpauing()
    {
        paused = false;
        audioManager.PlaySource();
        //if (LCData.loadingType == LCData.LoadingType.AudioFile) spectrumAsource.Play();
        ...
        while (timespeed < 1)
        {
            ...
            //if (LCData.loadingType == LCData.LoadingType.AudioFile) spectrumAsource.pitch = timespeed * pitch;
    ...
    public void SpawnBeatCube(BeatCubeClass beat)
    {
        ...
        c.transform.position = new Vector3(selectedSpawnPoint.position.x, beat.level == 0 ? 1 : 4f, selectedSpawnPoint.position.z);
