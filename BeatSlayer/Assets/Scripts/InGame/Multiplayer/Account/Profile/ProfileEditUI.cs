...
namespace Profile
{
    public class ProfileEditUI : MonoBehaviour
    {
        ...
        public void Open()
        {
            ...
            currentEmailText.text = (string.IsNullOrWhiteSpace(Payload.Account.Email)) ? "-" : Payload.Account.Email;
            newEmailCodeLine.SetActive(!string.IsNullOrWhiteSpace(Payload.Account.Email));
        }
        ...
        public void OnAvatarBtnClick()
        {
            ...
            ImageCropper.Instance.Show(tex, (result, image, croppedImage) =>
            {
                if (result)
                {
                    ...
                    Web.WebAPI.UploadAvatar(Payload.Account.Nick, croppedImage, message =>
                    {
            ...
            NativeGallery.GetImageFromGallery(new NativeGallery.MediaPickCallback(path =>
            {
                ...
                ImageCropper.Instance.Show(tex, (result, image, croppedImage) =>
                {
                    if (result)
                    {
                        ...
                        Web.WebAPI.UploadBackground(Payload.Account.Nick, croppedImage, message =>
                        {
        ...
        public void OnPasswordChangeBtnClick()
        {
            ...
            if (canContinue)
            {
                ...
                NetCore.ServerActions.Account.ChangePassword(Payload.Account.Nick, currentPasswordField.text, newpassword);
            }
        }
        public void OnEmailChangeBtnClick()
        {
            bool canContinue = true;
            bool isEmptyEmail = string.IsNullOrWhiteSpace(Payload.Account.Email);
            ...
            if (canContinue)
            {
                if (isEmptyEmail)
                {
                    ...
                    NetCore.ServerActions.Account.ChangeEmptyEmail(Payload.Account.Nick, newEmailField.text);
                }
                else NetCore.ServerActions.Account.ChangeEmail(Payload.Account.Nick, codeField.text);
            }
        }
        public void OnEmailChangeCodeBtnClick()
        {
            ...
            {
                newEmail = newEmailField.text;
                NetCore.ServerActions.Account.SendChangeEmailCode(Payload.Account.Nick, newEmail);   
            }
        }
